...
def make_example_and_histograms(line):
    ...
    for block_name, calls in nodes:
        for call in calls: # call is a tuple. starts with a function name (starts with A), and continues with an argument representations.
            api = call[0]
            api_counter += 1
            if api.startswith('N'):
            ...
            for arg in args: # arg is a pair. first is metadata, second is data.
                values = kind_utils.get_values_from_arg(arg)
                for v in values:
...
def make_example(obj):
    ...
    for node_name, node_calls in obj_nodes:
        if len(node_calls) == 0:
            ...
            for call in node_calls:
                ...
                args = call[1:]
                arg_vals = [kind_utils.get_values_from_arg(arg) for arg in args]
...
def process_file(file_path, data_file_role, dataset_name, collect_histograms=False):
    ...
    with open(file_path, 'r') as file:
        ...
        if collect_histograms:
            ...
            examples_with_histograms = map(make_example_and_histograms, file)
            for i, (ex, local_target_to_count, local_api_to_count, local_arg_to_count, local_num_nodes) \
                    in enumerate(examples_with_histograms):
                for key, val in local_target_to_count.items():
                    target_to_count[key] += val
